...
def estimateValues(data, lamb, lr, threshold=0.001):
	...
	for set in data:
		...
		while max(np.array(estimates) - np.array(prevEstimates)) > threshold: #checkEstimates(estimates, prevEstimates, threshold) == False:
...
def checkEstimates(est, prevEst, threshold):
	for ind in range(0,len(est)):
		if abs(est[ind] - prevEst[ind]) > threshold:
			return False
	return True
